...
import 'package:flutter_demo/components/callout.dart';
  ...
  void onTapDown() {
    if (!isDead) { // 如果还没被打死
      ...
      if (game.activeView == View.playing) {
        ...
        if (game.score > (game.storage.getInt("highscore") ?? 0)) { // 判断是否需要更新最高分
          game.storage.setInt("highscore", game.score);
          game.highscoreDisplay.updateHighscore();
        }
      }

      if (game.storage.getBool("soundenabled")?? true) { // 判断是否关闭了音效
        Flame.audio.play(
