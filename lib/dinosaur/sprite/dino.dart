...
class Dino extends Component {
  ...
  Dino(ui.Image spriteImage, this.size) {
    ...
    DinoRunConfig.list.forEach((DinoRunConfig config) {
      runSpriteList.add(
        Sprite.fromImage(spriteImage,
            x: config.x, y: yPos, width: DinoRunConfig.w, height: height),
      );
    });
    actualDinoList[1] = AnimationComponent(DinoRunConfig.w, height,
    ...
    DinoDownConfig.list.forEach((DinoDownConfig config) {
      downSpriteList.add(
        Sprite.fromImage(spriteImage,
            x: config.x, y: yPos, width: DinoDownConfig.w, height: height),
      );
    });
    actualDinoList[3] = AnimationComponent(DinoDownConfig.w, height,
